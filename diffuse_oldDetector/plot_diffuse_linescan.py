...
ROI_x_min = peak_fit_pump[0][0][1]-20
ROI_x_max = peak_fit_pump[0][0][1]+20
ROI_y_min = min_y-10
ROI_y_max = max_y+10
...
if x_flag ==1:
    ...
    ROI_y_min_2 = peak_fit_pump[0][1][1]-20
    ROI_y_max_2 = peak_fit_pump[0][1][1]+20
...
np.save(save_address + 'diffuse_vertical.npy', diffuse)
...
plt.figure()
for i in range(1,np.shape(runs)[0]):
    plt.plot(diffuse[i-1], label = str(int(round(delay[i]))) + ' ps')
